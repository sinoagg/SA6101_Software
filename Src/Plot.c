...
int PlotCurve(GraphTypeDef* pGraph, int hGraphPanel, int control)
{
	...
	{		 
	   	if(pGraph->pCurveArray->numOfPlotDots >= 1)										//如果一个点没画，只画一个点     
		{  																																																						 
...
{						
	...
	else //画第一个点
	{
			...
			{	
				...
				(pGraph2->pCurveArray + 1)->numOfPlotDots+=numOfDotsToPlot;		
				(pGraph2->pCurveArray + 1)->pDotXPlot+=numOfDotsToPlot;		
				(pGraph2->pCurveArray + 1)->pDotYPlot+=numOfDotsToPlot;			
				(pGraph2->pCurveArray + 1)->numOfDotsToPlot-=numOfDotsToPlot;	
				
				(pGraph2->pCurveArray + 2)->numOfPlotDots+=numOfDotsToPlot;		
				(pGraph2->pCurveArray + 2)->pDotXPlot+=numOfDotsToPlot;			
				(pGraph2->pCurveArray + 2)->pDotYPlot+=numOfDotsToPlot;			
				(pGraph2->pCurveArray + 2)->numOfDotsToPlot-=numOfDotsToPlot;	
			}
	}
