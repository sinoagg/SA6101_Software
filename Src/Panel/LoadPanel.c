...
int LoadInitPanel (void)
{
	...
	if ((hExpListPanel = LoadPanel (hMainPanel, "Experiment List.uir", EXP_LIST)) < 0)		//load left panel
		return -1;
	 if ((hImgPanel = LoadPanel (hMainPanel, "AutoSaveData.uir", IMGPANEL)) < 0)		//load left panel
		return -1;
	
	if ((hAutoSavePanelPanel = LoadPanel (hExpListPanel, "AutoSaveData.uir", AUTOPANEL)) < 0)		//load left panel              
			return -1;                                                                                                         
	...
	if ((hSaveDataPanel = LoadPanel (hMainPanel, "Result Menu.uir", SAVEDATA)) < 0)		//用于弹出savedata面板   
		return -1;
	if ((hSaveDataTipPanel = LoadPanel (hSaveDataPanel, "Result Menu.uir", SAVETIP)) < 0)		//用于弹出savedata面板   
		return -1;
	...
	SetPanelSize(hExpListPanel, 900, 300);//加载面板大小  (,height，width)	  hTestLogsPanel
	DisplayPanel(hExpListPanel);
	SetPanelPos(hGraphPanel, 172, 305);    //程序启动时最先加载的面板在最底层所以开始时加载graph面板
	DisplayPanel(hGraphPanel);
	
	
	SetPanelPos(hAutoSavePanelPanel, 426,0);  //加载面板位置 （,top,left）  i             
	SetPanelSize(hAutoSavePanelPanel,580, 297);//加载面板大小  (,height，width)	         
	DisplayPanel(hAutoSavePanelPanel);                                                    
