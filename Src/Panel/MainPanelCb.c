		...
		case EVENT_LEFT_CLICK_UP:
			...
			switch(TestPara.testMode)
			{
				...
				case NO_SWEEP_RT:
					 ...
					 Graph1.pCurveArray->numOfTotalDots  = numOfDots;
					 Table_init(table_title_IT, Table_ATTR.column, Table_ATTR.columnWidth);
					 CmtScheduleThreadPoolFunction (DEFAULT_THREAD_POOL_HANDLE, AbnmDCThreadFunction, NULL, &abnmDCThreadId); 
					 break;
							  ...
							  void *callbackData, int eventData1, int eventData2)
{
	switch (event)
	{
		case EVENT_LEFT_CLICK_UP:		    //当鼠标释放时
			...
			CmtWaitForThreadPoolFunctionCompletion (DEFAULT_THREAD_POOL_HANDLE, abnmDCThreadId, 0);
			break;
	}
	return 0;
}
...
{ 
	...
	{
		if(Graph1.pCurveArray->numOfPlotDots > 0 && Graph1.pCurveArray->numOfPlotDots >= (Graph1.pCurveArray->numOfTotalDots * n) / 100)  //每十分之一的总点数时更新一次缓存区
		{
			LaunchExcelCB();
