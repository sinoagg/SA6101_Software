...
//===============================20181117新增显示版本号功能；小马哥写===================================================================================================
		    ...
		    hardwareCircuitVersions =  IDAddrress[7] - '0') * 100 +  IDAddrress[8] * 10 + IDAddrress[9];
			embeddedSoftwareVersions =  IDAddrress[10] * 100 +  IDAddrress[11] * 10 + IDAddrress[12];
			mechanicalPowerVersions =  IDAddrress[13] * 100 +  IDAddrress[14] * 10 + IDAddrress[15];
...
//=======================================================================================================================================    
			...
			char ser[20] ;
			char str[20] ;
			sprintf(str,"%s%s","",currentSoftVersion);
			sprintf(ser,"%s%s","",serversSoftVersion);*/
			/*printf("%s",*str);
            SetCtrlAttribute(hSettingsAboutPanel, SETTINGS_VERSION,ATTR_CTRL_VAL,&ser);
			SetCtrlAttribute(hSettingsAboutPanel, SETTINGS_VERSION_2,ATTR_CTRL_VAL,&str);    */
	...
	switch (event)
	{
		case EVENT_LEFT_CLICK_UP:
			...
			if(temp == 1)
			{
				...
				if(updateFlag == -2)
				{
					//ProgressBar_SetPercentage(hSettingsAboutPanel,ABOUT_PROGRESSBAR,(double)100,"");
					SoftwareChange(0);
					return 0;
				}
				CVIXMLLoadDocument (localInfoPath, &hLoDocument);
