...
{						
	...
	GetSettingsCurveAttr(pGraph2->graphIndex,pGraph2->pCurveArray->curveIndex);
	...
	else //画第一个点
	{
			...
			{	
					if(temp_flag == 1)
				{
					Graph2.plotHandle=PlotXY(hGraphPanel, control, (Graph2.pCurveArray +1)->pDotXPlot, (Graph2.pCurveArray +1)->pDotYPlot, numOfDotsToPlot, VAL_FLOAT, VAL_FLOAT, pCurveAttr->plotStyle,
									  		   pCurveAttr->pointStyle,
									  		   pCurveAttr->lineStyle, 1, graph2tempclr);
				}
				
				if(humidity_flag == 1) 
				{
					Graph2.plotHandle=PlotXY(hGraphPanel, control, Graph2.pCurveArray->pDotXPlot, Graph2.pCurveArray->pDotYPlot, numOfDotsToPlot, VAL_FLOAT, VAL_FLOAT,pCurveAttr->plotStyle,
									  		   pCurveAttr->pointStyle,
									  		   pCurveAttr->lineStyle, 1, graph2humclr);
				}
				if(pressure_flag == 1)
				{
					Graph2.plotHandle=PlotXY(hGraphPanel, control, (Graph2.pCurveArray +2)->pDotXPlot, (Graph2.pCurveArray +2)->pDotYPlot, numOfDotsToPlot, VAL_FLOAT, VAL_FLOAT,pCurveAttr->plotStyle,
									  		   pCurveAttr->pointStyle,
									  		   pCurveAttr->lineStyle,1, graph2preclr);
				}
				pGraph2->pCurveArray->numOfPlotDots+=numOfDotsToPlot;		//画图总点数递增
				pGraph2->pCurveArray->pDotXPlot+=numOfDotsToPlot;			//画图点X坐标指针递增
				pGraph2->pCurveArray->pDotYPlot+=numOfDotsToPlot;			//画图点Y坐标指针递增
...
void DisplayTempGraph()
{
	...
	if((temp_flag == 1)&&((Graph2.pCurveArray  + 1)->numOfPlotDots>0))  
	{
		...
		PlotXY(hGraphPanel, GRAPHDISP_GRAPH2, (Graph2.pCurveArray  + 1)->pDotXHead, (Graph2.pCurveArray  + 1)->pDotYHead, (Graph2.pCurveArray  + 1)->numOfPlotDots, VAL_FLOAT, VAL_FLOAT, pCurveAttr->plotStyle,
									  		   pCurveAttr->pointStyle,
									  		   pCurveAttr->lineStyle, 1, graph2tempclr);
	}
	else
	...
	{						 
		...
		PlotXY(hGraphPanel, GRAPHDISP_GRAPH2, Graph2.pCurveArray->pDotXHead, Graph2.pCurveArray->pDotYHead, Graph2.pCurveArray->numOfPlotDots, VAL_FLOAT, VAL_FLOAT,pCurveAttr->plotStyle,
									  		   pCurveAttr->pointStyle,
									  		   pCurveAttr->lineStyle, 1, graph2humclr);
	...
	if((pressure_flag == 1)&&((Graph2.pCurveArray  + 2)->numOfPlotDots>0)) 
	{
		...
		PlotXY(hGraphPanel, GRAPHDISP_GRAPH2, (Graph2.pCurveArray  + 2)->pDotXHead, (Graph2.pCurveArray  + 2)->pDotYHead, (Graph2.pCurveArray  + 2)->numOfPlotDots, VAL_FLOAT, VAL_FLOAT, VAL_CONNECTED_POINTS, VAL_DOTTED_SOLID_SQUARE, VAL_SOLID, 1, graph2preclr);
