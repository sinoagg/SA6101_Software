...
static void GetTestPara(ExpPanelTypeDef* pExpPanel, TestParaTypeDef* pTestPara)
{
	...
	//GetCtrlVal(hBasicSamplePanel, SAMPLE_ADV_TIMESTEP, &(pTestPara->timeStep));
	GetCtrlVal(hBasicSamplePanel, SAMPLE_CFG_RUNTIME, &(pTestPara->runTime));
	GetCtrlVal(hAdvanceSamplePanel, SAMPLE_ADV_SAMPLERATE, &(pTestPara->sampleRate));
	GetCtrlVal(hAdvanceSamplePanel, SAMPLE_ADV_SAMPLENUMBER, &(pTestPara->sampleNumber));
	...
	GetCtrlVal(hBasicSamplePanel, SAMPLE_CFG_SAMPLEDELAY, &(pTestPara->SampleDelay));	 //获取采样延时
}
...
static void PrepareCfgTxData(TestParaTypeDef* pTestPara, unsigned char devAddr, unsigned char* pmeasUartTxBuf,unsigned char connectionType)
{
	*pmeasUartTxBuf=devAddr;
	*(pmeasUartTxBuf+1)=MSG_TYPE_SETTING;     
	*(pmeasUartTxBuf+2)=(unsigned char)(pTestPara->testMode);
	*(pmeasUartTxBuf+3)=connectionType;   
	...
	*(pmeasUartTxBuf+29)=(unsigned char)((pTestPara->SampleDelay)>>8);  //采样延时
	*(pmeasUartTxBuf+30)=(unsigned char)((pTestPara->SampleDelay)&0xFF);
	
	
	*(pmeasUartTxBuf+SA61_UART_TX_LEN-1)=GetXorCheckVal(pmeasUartTxBuf, SA61_UART_TX_LEN-1); 
}

void ProtocolCfgTxData(unsigned char comSelect, unsigned char devAddr, unsigned char* pmeasUartTxBuf,unsigned char connectionType)
{
	PrepareCfgTxData(&TestPara, devAddr, pmeasUartTxBuf, connectionType);      
...
void ProtocolCfg(unsigned char comSelect, unsigned char devAddr, enum TestMode expType, unsigned char* pmeasUartTxBuf,unsigned char connectionType)
{
	switch(expType)
	{
	...
	ProtocolCfgTxData(comSelect, devAddr, pmeasUartTxBuf,connectionType);
}
...
void ProtocolGetData(unsigned char* pmeasUartRxBuf, RxDataTypeDef* pRxData)	//Get data from UART Rx Buffer
{
	...
	TestPara.StopSign = pRxData->rxStopSign;								   //
	...
	pRxData->realTime=*(pmeasUartRxBuf+19);   //真实采样时间  
	pRxData->realTime<<=8;
	pRxData->realTime|=*(pmeasUartRxBuf+20);   
	pRxData->realTime<<=8;
	pRxData->realTime|=*(pmeasUartRxBuf+21);
	pRxData->realTime<<=8;
	pRxData->realTime|=*(pmeasUartRxBuf+22);            
}
