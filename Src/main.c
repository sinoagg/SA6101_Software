...
{
	int temp,hum,press;
	GetCtrlVal(hEnvResultPanel,ENVIRPANEL_TEMP_BOX,&temp);		//然后读取用户要选中的曲线
	GetCtrlVal(hEnvResultPanel,ENVIRPANEL_HUM_BOX,&hum); 
	GetCtrlVal(hEnvResultPanel,ENVIRPANEL_PRESS_BOX,&press);
	switch(TestPara.testMode)
	{
		//case SWEEP_DRAIN_VOL:
		//case SWEEP_GATE_VOL :
...
int main (int argc, char *argv[])
{
	...
	RunUserInterface();
		...
		if(SA11_UART_Status==2)				//如果电学和环境的测量设备都打开的话，结束时需要同时关闭
		{
			CloseCom(controlComPort);
			CloseCom(measureComPort);
		}
